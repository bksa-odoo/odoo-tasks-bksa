...
class SaleOrder(models.Model):
    ...
    total_commission = fields.Float('Total Commission', compute="_compute_total_commission", store=True)
    
    @api.depends('order_line.commission_lines')
    def _compute_total_commission(self):
        for order in self:
            total_commission = sum(line.commission_lines for line in order.order_line)
    ...
    def _compute_amounts(self):
        ...
        for order in self:
            ...
            if order.commission:
                order.amount_tax += order.total_commission
